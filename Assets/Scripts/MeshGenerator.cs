		...
		for(int y = 0; y < numberOfVerticesPerLine; y++) {
			for(int x = 0; x < numberOfVerticesPerLine; x++) {
				...
				if(!isSkippedVertex) {
					...
					Vector2 vertextPosition2D = topLeft + new Vector2(percent.x, -percent.y) * meshSettings.meshWorldSize;
					float height = heightMap[x, y];


					if(isEdgeConnectionVertex) {
						bool isVertical = x == 2 || x == numberOfVerticesPerLine - 3;
						// Distance to the closest vertex above/before the edge connection vertex
						int distanceToMainVertexA = (isVertical ? y - 2 : x - 2) % skipIncrement;
						// Distance to the closest vertex below/after the edge connection vertex
					...
					meshData.AddVertex(new Vector3(vertextPosition2D.x, height, vertextPosition2D.y), percent, vertexIndex);
...
public class MeshData {
	...
	public MeshData(int numberOfVerticesPerLine, int skipIncrement, bool useFlatshading) {
		...
		int numberOfMainTriangles = (numberOfMainVertices - 1) * (numberOfMainVerticesPerLine - 1) * 2;
		triangles = new int[(numberOfMeshEdgeVertices + numberOfMainTriangles) * 3];
	...
	Vector3[] CalculateNormals() {
		...
		for(int i = 0; i < triangleCount; i++) {
			...
			Vector3 trianlgeNormal = SurfaceNormalFromIndices(vertexIndexA, vertexIndexB, vertexIndexC);
			vertexNormals[vertexIndexA] += trianlgeNormal;
			vertexNormals[vertexIndexB] += trianlgeNormal;
			vertexNormals[vertexIndexC] += trianlgeNormal;
		}
